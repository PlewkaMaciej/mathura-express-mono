...
export default function QuestionWithAnswers({
  ...
  useEffect(() => {
    ...
    const fetchDbUserId = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Nie znaleziono użytkownika w bazie");
  ...
  useEffect(() => {
    ...
    const updateQuestion = () => {
      ...
      if (!selectedQuestion.isPublic && !dbUserId) {
        setQuestion(null);
        setAnswers([]);
        return;
      }
    ...
    updateQuestion();
    const interval = setInterval(updateQuestion, 10_000);
    return () => clearInterval(interval);
  }, [questions, questionParam, dbUserId]);
